...
func TestCodeGenAST(t *testing.T) {
	...
	nodespecs := []nodespec{
		{"NodeLiteral", []fieldspec{{"Text", "string"}, {"Span", "Span"}}, "n.Span"},
		{"NodeGoStrExpr", []fieldspec{{"Expr", "string"}, {"Span", "Span"}}, "n.Span"},
		{"NodeGoCode", []fieldspec{{"Context", "GoCodeContext"}, {"Code", "string"}, {"Span", "Span"}}, "n.Span"},
		...
		{"NodePartial", []fieldspec{{"Name", "string"}, {"Span", "Span"}, {"Block", "*NodeBlock"}}, "n.Span"},
		{"NodeBlock", []fieldspec{{"Nodes", "[]Node"}}, "n.Nodes[0].Pos()"},
		{"NodeElement", []fieldspec{{"Tag", "Tag"}, {"StartTagNodes", "[]Node"}, {"Children", "[]Node"}, {"Span", "Span"}}, "n.Span"},
		{"NodeImport", []fieldspec{{"Decl", "ImportDecl"}, {"Span", "Span"}}, "n.Span"},
		// TODO NodeList
	}
	...
	originalText, err := os.ReadFile("ast.go")
	if err != nil {
		t.Fatal(err)
	}
	...
	if !found {
		t.Fatalf("could not find %q in ast.go", beginMarker)
	}

	_, after, found = strings.Cut(after, endMarker)
	if !found {
		t.Fatalf("could not find %q in ast.go", endMarker)
	}
...
`
	...
	if !bytes.Equal(originalText, src) {
		...
		// to the target
		if err := os.WriteFile("ast.go.tmp", src, 0644); err != nil {
			t.Fatal(err)
		}
		if err := os.Rename("ast.go.tmp", "ast.go"); err != nil {
			t.Fatal(err)
		}
